...
import {
  Star, X, CheckCircle2, Info, Loader2, PawPrint as Paw, Crown,
} from "lucide-react";
...
function PuppyCard({ puppy, onClick }: { puppy: Puppy; onClick: () => void }) {
  ...
  return (
    <div
      className={`group bg-card rounded-2xl overflow-hidden border shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-2 cursor-pointer ${puppy.isPremium ? "border-yellow-400/50 shadow-yellow-400/10" : "border-border/50"}`}
      onClick={onClick}
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {puppy.isPremium && (
          <div className="absolute top-0 left-0 right-0 flex justify-center pt-3">
            <div className="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-gradient-to-r from-yellow-500 to-amber-400 text-white text-xs font-bold shadow-lg">
              <Crown className="w-3.5 h-3.5" /> Annonce Premium
            </div>
          </div>
        )}
        <div className={`absolute ${puppy.isPremium ? "top-14" : "top-3"} left-3`}>
...
export default function Chiots() {
  ...
  const premiumPuppies = filtered.filter((p) => p.isPremium);
        ...
        <section className="py-12">
          <div className="container px-4 mx-auto">
            ...
            {isLoading ? (
              ...
              <>
                {/* Premium section */}
                {premiumPuppies.length > 0 && (
                  <div className="mb-14">
                    <div className="flex items-center gap-3 mb-6">
                      <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-yellow-500 to-amber-400 text-white text-sm font-bold shadow-md">
                        <Crown className="w-4 h-4" /> Annonces Premium
                      </div>
                      <div className="flex-1 h-px bg-gradient-to-r from-yellow-400/30 to-transparent" />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                      {premiumPuppies.map((p) => <PuppyCard key={p.id} puppy={p} onClick={() => setSelectedPuppy(p)} />)}
                    </div>
                  </div>
                )}
                ...
                {regularPuppies.length > 0 && (
                  <div>
                    {premiumPuppies.length > 0 && (
                      <div className="flex items-center gap-3 mb-6">
                        <span className="text-sm font-medium text-muted-foreground">Toutes les annonces</span>
      ...
      {selectedPuppy && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-card text-card-foreground w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <button onClick={() => setSelectedPuppy(null)} className={`absolute right-4 z-20 w-10 h-10 bg-background/60 backdrop-blur-md rounded-full flex items-center justify-center hover:bg-background transition-colors ${selectedPuppy.isPremium ? "top-12" : "top-4"}`}>
              <X className="w-5 h-5" />
            </button>
